...
void view_dashboard(void) {
    ...
    display_time();
    clcd_print(event[event_index], (0xC0 + (9)));

    key_press = read_switches(1);
    ...
    else
    {
        ...
        adc_val = (unsigned int) (read_adc(0x04) / 10.33);



        clcd_putch(((adc_val / 10) + 48), (0xC0 + (12)));

        clcd_putch(((adc_val % 10) + 48), (0xC0 + (13)));
    }
}
...
void event_store(void)
{
    ...
    if(event_count > 10 - 1)
    {
        ...
        {
            unsigned char copy_from = start_addr + (i * 12);
            unsigned char copy_to = start_addr + ((i - 1) * 12);
        ...
        EEPROM_addr = start_addr + ((10 - 1) * 12);
    }
    else
    {

        EEPROM_addr = start_addr + (event_count * 12);
        event_count++;
    }

    write_event(EEPROM_addr);
}
